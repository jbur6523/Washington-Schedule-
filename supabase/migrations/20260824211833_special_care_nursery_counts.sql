alter table public.shift_status_updates
  add column neonatal_high_flow_count integer,
  add column bubble_cpap_count integer;

alter table public.shift_status_updates
  add constraint shift_status_updates_neonatal_high_flow_count_nonnegative
    check (neonatal_high_flow_count is null or neonatal_high_flow_count >= 0),
  add constraint shift_status_updates_bubble_cpap_count_nonnegative
    check (bubble_cpap_count is null or bubble_cpap_count >= 0);
...
begin
  ...
  if found then
    ...
    set
      ...
      neonatal_high_flow_count = case
        when shift_payload ? 'neonatal_high_flow_count' and shift_payload -> 'neonatal_high_flow_count' <> 'null'::jsonb
          then (shift_payload ->> 'neonatal_high_flow_count')::integer
        else current_update.neonatal_high_flow_count
      end,
      bubble_cpap_count = case
        when shift_payload ? 'bubble_cpap_count' and shift_payload -> 'bubble_cpap_count' <> 'null'::jsonb
          then (shift_payload ->> 'bubble_cpap_count')::integer
        else current_update.bubble_cpap_count
      end,
    ...
    values (
      ...
      case when shift_payload -> 'neonatal_high_flow_count' = 'null'::jsonb then null else (shift_payload ->> 'neonatal_high_flow_count')::integer end,
      case when shift_payload -> 'bubble_cpap_count' = 'null'::jsonb then null else (shift_payload ->> 'bubble_cpap_count')::integer end,
