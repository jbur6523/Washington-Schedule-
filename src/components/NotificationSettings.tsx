...
export function NotificationSettings({ authContext, developmentFallback }: NotificationSettingsProps) {
  ...
  const enableNotifications = async () => {
    ...
    if (!supported) {
      setError("Notifications are not supported on this device. You can still check the Shift Board manually.");
      return;
    }
    ...
    if (nextPermission !== "granted") {
      setSaving(false);
      setError("Notifications are not enabled on this device. You can still check the Shift Board manually.");
      return;
    }
