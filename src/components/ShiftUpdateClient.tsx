...
function shiftUpdateFormForSelection(
  ...
  return {
    ...
    neonatalHighFlowCount: update?.neonatal_high_flow_count === null || update?.neonatal_high_flow_count === undefined
      ? "0"
      : String(update.neonatal_high_flow_count),
    bubbleCpapCount: update?.bubble_cpap_count === null || update?.bubble_cpap_count === undefined
      ? "0"
      : String(update.bubble_cpap_count),
  ...
  return (
    <label className="flex min-h-[9.75rem] flex-col items-center justify-between rounded-2xl border border-slate-100 bg-slate-50/80 px-2.5 py-3 text-center shadow-sm">
      ...
      <input
        ...
        onChange={(event) => onChange(event.target.value)}
        className="mt-2 h-11 w-full rounded-2xl border border-slate-400 bg-white px-2 text-center text-3xl font-black leading-none text-hospital-ink shadow-sm outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400"
      />
...
export function ShiftUpdateClient({
  ...
  const loadSelectedShiftUpdate = useCallback(async () => {
    ...
    const supabase = createClient();
    const { data: selectedUpdate, error: updatesError, usedLegacyNurserySelect } = await fetchShiftStatusUpdateForRecord(
      supabase,
      authContext.departmentId,
      selection.shiftDate,
      selection.shiftType
    ...
    setNurseryTrackingAvailable(!usedLegacyNurserySelect);
  ...
  const printData: ShiftStatusPrintData = {
    ...
    bubbleCpap: form.bubbleCpapCount,
  ...
  const persistShiftUpdate = async (printAfterSave: boolean) => {
    ...
    const normalizedForm = withDefaultTrackedCounts(form);
    ...
    const basePayload = {
      ...
      ...(nurseryTrackingAvailable
        ? {
            neonatal_high_flow_count: shiftStatusNumberValue(normalizedForm.neonatalHighFlowCount),
            bubble_cpap_count: shiftStatusNumberValue(normalizedForm.bubbleCpapCount)
          }
        : {}),
        ...
        <form onSubmit={saveShiftUpdate} className="space-y-4">
          ...
          <section className="rounded-3xl border border-white bg-white/95 p-4 shadow-soft">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-black text-hospital-ink">Special Care Nursery</h2>
                <p className="mt-1 text-xs font-bold text-slate-500">Respiratory support for this shift</p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2.5">
              <ProcedureInputTile
                ...
                value={form.neonatalHighFlowCount}
                helperText={lastKnownHelper(lastKnownUpdate, lastKnownUpdate?.neonatal_high_flow_count, timezone)}
                onChange={(value) => setForm((current) => ({ ...current, neonatalHighFlowCount: value }))}
                disabled={!nurseryTrackingAvailable}
              />
              <ProcedureInputTile
                icon={<BubbleCpapIcon className="h-[18px] w-[18px]" />}
                label="Bubble CPAP"
                value={form.bubbleCpapCount}
                helperText={lastKnownHelper(lastKnownUpdate, lastKnownUpdate?.bubble_cpap_count, timezone)}
                onChange={(value) => setForm((current) => ({ ...current, bubbleCpapCount: value }))}
                disabled={!nurseryTrackingAvailable}
              />
            </div>
            {!nurseryTrackingAvailable && (
              <p className="mt-3 rounded-2xl border border-amber-100 bg-amber-50 px-3 py-2 text-xs font-bold leading-5 text-amber-800">
                Nursery tracking will be available after the database update is applied.
              </p>
            )}
          </section>
