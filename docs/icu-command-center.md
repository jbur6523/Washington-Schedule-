...
Vent cards show a compact top-right overflow action with a `Vent Status` section. Its persistent status toggles are SBT, Critical, Flolan, Prone, and Standby. Critical reuses `icu_patients.is_critical_vent`; Standby reuses `icu_patients.is_standby`; SBT, Flolan, and Prone use additive boolean fields on the same active ICU row. They persist across refreshes, devices, logins, and shift boundaries until manually cleared. Critical gives the card an accessible red treatment, Standby gives a non-critical card an accessible yellow treatment, and SBT gives a non-critical, non-Standby card an accessible blue treatment. Active SBT, Proned, On Flolan, and Standby modifiers remain visible on a separate status line. Vent cards do not show an inactive `Not Standby` pill.
