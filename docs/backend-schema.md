...
- `lead_communication_notes`: department-scoped operational notes for RT Leads/resources. Each row stores note text, Normal/Urgent priority, New/Reviewed/Closed status, visible creator display name, review metadata, optional one-time follow-up note metadata, optional close metadata, and timestamps. Note and follow-up text are capped at 500 characters.
...
- Lead Communication Board RLS allows Admin, Lead, Respiratory Command Center, Director, and ICU Command Center users to create/view notes. Only Admin and Lead users can mark notes reviewed or add follow-up notes. Staff, Aide, and unauthenticated users are denied unless they also have an allowed role.
