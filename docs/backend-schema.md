...
- `rental_records`: active and historical rental records. `pending_delivery` / `called_in` records display as blue `Pending Delivery`, can exist before a serial number is known, and do not count as Active Rentals. Delivery confirmation updates the pending row to `active`. `checked_in_at` is retained as the delivered timestamp for compatibility and displays as `Delivered` in the UI. `pickup_requested_at`, `pickup_requested_by_staff_profile_id`, `pickup_confirmation_number`, and `pickup_request_note` support the Called for Pickup step. `returned_at`, `returned_by_staff_profile_id`, and `return_note` support Picked Up confirmation. `active` / `delivered` records display green, `pickup_requested` / `pickup_called` records display yellow, and `returned` / `picked_up` records display as gray `Picked Up`.
- `rental_events`: audit-style rental events such as `called_in`, `delivered`, `checked_in`, `manual_check_in`, `barcode_scanned`, pickup-call events, `returned` / `picked_up`, and future `transferred` events. Rental History can use events for timeline details and picked-up-by display when those events exist.
- Rental notes are capped at 140 characters and must not contain patient information.
- Rental History export is generated server-side from `rental_records`, `rental_vendors`, `rental_events`, and related `staff_profiles` display names. The database remains the source of truth; exported CSV files are manual paper-trail copies only and do not include usernames, auth IDs, staff phone numbers, staff emails, patient information, or clinical details.
