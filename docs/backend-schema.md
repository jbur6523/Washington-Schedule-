...
- ICU history supports each card's `History` action, the `Today’s ICU Activity` section, and the `Search Previous Date` workflow. It is an operational paper-trail substitute only and must not include patient identifiers.
