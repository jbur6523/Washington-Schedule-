...
- `rental_records`: active and historical rental records. `pending_delivery` / `called_in` records display as blue `Pending Delivery`, can exist before a serial number is known, and do not count as Active Rentals. Delivery confirmation updates the pending row to `active`. `checked_in_at` is retained as the delivered timestamp for compatibility and displays as `Delivered` in the UI. `active` / `delivered` records display green, `pickup_requested` / `pickup_called` records display yellow, and `returned` / `picked_up` records display as gray `Picked Up`.
