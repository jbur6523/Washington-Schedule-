...
- `rt_aide_notes`: department-scoped notes and questions from Respiratory Command Center/Lead/Admin users to RT Aides. Each row stores note text, Normal/Urgent priority, New/Acknowledged/Responded/Closed status, visible creator display name from the required `Added by` field, optional acknowledgment metadata, optional response text/metadata, optional close metadata, and timestamps. Note and response text are capped at 500 characters. The internal `created_by_staff_profile_id` remains the authenticated profile for existing RLS checks.
