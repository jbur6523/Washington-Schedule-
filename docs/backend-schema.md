...
- `shift_status_updates`: Command Center shift-level operational updates. Rows store manually entered RTs On Shift in `rts_on`, exact raw RVUs in nullable `rvu_total`, server-derived RTs Needed in `rts_required`, optional vent count, BiPAP count, nullable Special Care Nursery counts in `neonatal_high_flow_count` and `bubble_cpap_count`, scheduled procedure counts including C-Sections, Vaginal Delivery, CABG, Bronchs, Sputum Inductions, and MRI, an optional `shift_note` operational note capped at 500 characters, plus visible updated-by attribution for the Director Shift Status page and compact Schedule summary. Nursery columns are nullable so pre-feature history remains missing rather than becoming a fabricated zero; new Shift Updates submit explicit counts, including valid zeroes. Existing rows retain null RVUs and notes; RVUs are never reconstructed from rounded staffing values. A null Vent field means no Vent change; zero remains a valid update. MRI is stored in the existing `other_procedure_count` column for compatibility. Director Scheduled Procedures display the latest saved procedure counts for 24 hours, then show `0` while preserving historical rows. `rts_required` is calculated as `round(rvu_total / 27, 1)` when RVUs exist.
- `official_vent_count_updates`: append-only canonical department Vent updates with operational date and day/night shift retained as audit metadata. Each row stores the official count, `lead_command_center` or `icu_command_center` source, server-generated field-update timestamp, and updater attribution when available. The newest department event by `created_at, id` is the current value across all shifts and dates. Authenticated department members can read it but cannot write it directly. Database triggers append only after a genuine non-null Lead Vent-field change or persisted ICU active-Vent membership change.
...
- `rt_aide_notes`: department-scoped notes and questions from Respiratory Command Center/Lead/Admin users to RT Aides. Each row stores note text, Normal/Urgent priority, New/Acknowledged/Responded/Closed status, visible creator display name from the required RT-side `Added by` field, optional acknowledgment metadata, optional response text/metadata from the Aide-side `Added by` field, optional close metadata, and timestamps. Note and response text are capped at 500 characters. The internal `created_by_staff_profile_id` and `responded_by_staff_profile_id` remain the authenticated profile for existing RLS checks.
- `lead_communication_notes`: department-scoped operational notes for RT Leads/resources. Existing `new`/`reviewed` status values provide one shared unread/read state; opening the Lead/Admin board acknowledges only rows created by that entry time, while `Mark Unread` restores `new`. Historical review metadata remains stored. Rows also retain Normal/Urgent priority, creator attribution, optional one-time reply metadata, optional close metadata, and timestamps. Note and reply text are capped at 500 characters.
...
- Aide Communication Board RLS allows Admin, Lead, and Respiratory Command Center users to create and view notes. Aide and Admin users can acknowledge and respond from Order Management. Staff, Director, ICU Command Center, and unauthenticated users are denied.
- Lead Communication Board RLS allows Admin, Lead, Respiratory Command Center, Director, Leadership, and ICU Command Center users to create/view notes. Admin and Lead users manage the shared read/unread status. Admin, Lead, and Leadership users can add the existing one-time reply; replies do not change read state. Staff, Aide, and unauthenticated users are denied unless they also have an allowed role.
...
- ICU history supports each card's `History` action, the `Today’s ICU Activity` section, and the `Search Previous Date` workflow. Vent SBT, Critical, Flolan, and Prone changes are committed atomically with their audit rows. CT/MRI are permanent board-update events keyed to the America/Los_Angeles 07:00/19:00 operational shift; a partial unique index prevents repeat taps in the same shift without deleting history. It is an operational paper-trail substitute only and must not include patient identifiers.
...
- Rental lifecycle mutations use guarded database functions: `create_pending_rental_delivery`, `confirm_rental_delivery`, `call_rental_pickup`, `cancel_rental_pickup`, `confirm_rental_picked_up`, and `cancel_rental_delivery`. Each function verifies the current rental status, confirms the user has Rental Management access, verifies active staff attribution, performs the status update, and inserts the corresponding event only after the guarded update succeeds. Stale-state attempts raise a transition error and do not insert duplicate events.
...
- Rental History export is generated server-side from `rental_records`, `rental_vendors`, `rental_events`, and related `staff_profiles` display names. The database remains the source of truth; exported CSV files include separate Barcode # and Serial Number columns and do not include usernames, auth IDs, staff phone numbers, staff emails, patient information, or clinical details. The Excel sync feed at `/api/rental-history/excel-feed` uses the same paper-log columns, does not require browser session cookies, and is protected by the server-only `RENTAL_EXCEL_SYNC_TOKEN`.
