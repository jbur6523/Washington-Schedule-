...
The Director should set or choose his own password through the normal password setup/reset process. The page is the primary live visual reporting dashboard for respiratory shift status. It uses a compact executive layout with visible `Refresh` and `Sign Out` controls, a `Current Shift Status` card for Scheduled and RTs Needed, a `Department Snapshot` card for Vents, BiPAPs, and scheduled procedure total, scheduled procedure detail cards, last updated freshness text, and updated-by display name/initials. The Director can switch between Today Day Shift, Today Night Shift, Tomorrow Day Shift, Tomorrow Night Shift, and Previous Shift from a compact selector. The Current Shift Status card also includes `View Shift`, which opens a read-only modal schedule preview for uploaded dates and Day/Night shifts. The modal defaults to the current `America/Los_Angeles` date when uploaded, falls back to the closest upcoming uploaded date or most recent uploaded date, and defaults to Day Shift from `07:00-18:59` Pacific or Night Shift from `19:00-06:59` Pacific. The modal uses its top `Close` control and does not show redundant bottom `Refresh` or `Close` buttons. The page remains read-only and does not allow editing.
