...
The Director should set or choose his own password through the normal password setup/reset process. The page is the primary live visual reporting dashboard for respiratory shift status. It uses a compact executive layout with visible `Sign Out`, a `Respiratory Directory` action, a `Current Shift Status` card for Scheduled and RTs Needed, a `Department Snapshot` card for Vents, BiPAPs, scheduled procedure total, Active Rentals, centered shift/date context, and last-updated metadata, plus scheduled procedure detail cards with centered shift/date context. The `Respiratory Directory` modal includes all active and inactive staff names and phone numbers from `staff_profiles`, sorted by display name, without usernames, auth IDs, emails, edit buttons, or admin controls. The Current Shift Status card also includes `View Shift`, which opens a read-only modal schedule preview for uploaded dates and Day/Night shifts. The modal defaults to the current `America/Los_Angeles` date when uploaded, then yesterday if today is unavailable. Its date dropdown only shows today/yesterday by default, and older uploaded dates can be entered manually as `MMDDYY`. View Shift defaults to Day Shift from `07:00-18:59` Pacific or Night Shift from `19:00-06:59` Pacific. Scheduled Procedures reset to `0` at `07:30` and `19:30` Pacific until a new Command Center update is submitted after the reset boundary, while Department Snapshot continues showing latest known Vent, BiPAP, and Active Rentals numbers. The page remains read-only and does not allow editing.
