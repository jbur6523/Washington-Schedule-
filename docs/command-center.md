...
The Director page includes a dashboard-specific `DirectorDashboardIcuSummary` section with Vents, HFNC, BiPAP, and Critical Vents. Its composed view model takes Vents only from the exact same official Vent state as Department Snapshot; HFNC, BiPAP, Critical Vents, detail rows, and their ICU-specific metadata continue to come from raw ICU tracking. The Director Vent tile has no raw-ICU fallback during loading or errors. `View All` opens a read-only ICU detail report with bed, device, airway when relevant, settings, Critical Vent flag, and last updated time. Director users cannot edit, discontinue, toggle Critical Vent status, or view internal ICU lifecycle controls.
...
Scheduled Procedures entered from the Lead Command Board Shift Update display on the Director Dashboard Scheduled Procedures card and in the Department Snapshot scheduled procedure total. Procedure counts remain visible for 24 hours after the latest Command Center update. If no procedure update exists, or the latest procedure update is older than 24 hours, the Director Dashboard displays `0` for all procedure counts without deleting historical shift update rows. The Scheduled Procedures card still shows last-updated metadata and marks stale data as expired.
